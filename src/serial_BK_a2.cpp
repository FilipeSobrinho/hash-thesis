// serial_BK_a2.cpp
// Bottom-k accuracy, single-threaded, A2 dataset.
...
int main(int argc, char** argv) {
    ...
    std::size_t K = 24'500;
    std::size_t R = 50'000;
    std::string outfile = "bottomk_a2_relerr.csv";
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--items" || arg == "--D") ITEMS = std::stoull(next());
            ...
            std::cout << "Usage: serial_BK_a2 [--items 500000] [--k 24500] [--R 1000] [--out file.csv]\n";
            return 0;
        }
    }

    std::cout << "Bottom-k accuracy (A2) single-threaded\n"
              << "  items=" << ITEMS << "  k=" << K << "  R=" << R << "\n"
              << "Writing: " << outfile << "\n";
