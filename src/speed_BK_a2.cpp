...
// speed_BK_a2.cpp
// Measure time to INSERT all A2 keys into BottomK, for multiple hash families.
int main(int argc, char** argv) {
    try {
        std::size_t loops = 1;
        std::size_t K = 24500;
        std::string out_csv = "a2_speed_bk.csv";
        for (int i = 1; i < argc; ++i) {
            ...
            if (a == "--loops") loops = std::stoull(next());
                ...
                std::cout << "Usage: speed_BK_a2 [--loops L] [--K K] [--out file.csv]\n";
                return 0;
            }
        }
