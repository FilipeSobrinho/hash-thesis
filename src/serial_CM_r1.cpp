// serial_CM_r1.cpp
// Single-threaded CMS on R1 with TornadoOnMSVec D1..D4
#include <algorithm>
#include <array>
#include <cstdint>
#include <cstring>
...
int main(int,char**){
  size_t WIDTH=32768,DEPTH=3,R=50000; std::string outfile="cms_r1_relerr.csv";
  ...
  std::unordered_map<Key20,uint32_t,Key20Hash> freq; freq.reserve(N); for(size_t i=0;i<N;++i){ Key20 k; std::memcpy(k.b.data(), raw.data()+i*20,20); ++freq[k]; }
  std::vector<Key20> distinct; distinct.reserve(freq.size()); for(auto&kv:freq) distinct.push_back(kv.first);
  ...
  auto mre=[&](const sketch::CountMin& cms, auto&& h32)->double{ long double s=0; for(const auto& k:distinct){ uint32_t t=freq.find(k)->second; uint32_t est=cms.estimate(h32(k.b.data(),20)); s += ((long double)est - (long double)t) / (long double)t; } return (double)(s/(long double)distinct.size()); };
  for(size_t r=0;r<R;++r){
    ...
    for(int f=0;f<NUM;++f){ sketch::CountMin cms(WIDTH,DEPTH); std::vector<RowHash32> rows=V[r].rows; for(size_t d=0;d<DEPTH;++d) cms.set_row(d,&rows[d]);
      for(size_t i=0;i<N;++i){ const void*p=raw.data()+i*20; cms.add(fam(f,p,20),1); } out<<NAMES[f]<<","<<(r+1)<<","<<mre(cms,[&](const void*p,size_t l){return fam(f,p,l);})<<"\n"; }
  } std::cout<<"Done.\n"; return 0;
}