// serial_CM_a1.cpp
// Count–Min Sketch accuracy, single-threaded, A1 dataset.
...
int main(int argc, char** argv) {
    ...
    std::size_t DEPTH = 3;
    std::size_t R = 50'000;
    std::string outfile = "cms_a1_relerr.csv";
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--items" || arg == "--D") ITEMS = std::stoull(next());
            ...
            std::cout << "Usage: serial_CM_a1 --items 500000 --width 32768 --depth 3 --R 1000 --out cms_a1_relerr.csv\n";
            return 0;
        }
    }

    std::cout << "CMS accuracy (A1) single-threaded\n"
